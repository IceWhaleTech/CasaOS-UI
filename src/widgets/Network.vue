  ...
  methods: {
    buildDatas(data) {
      ...
      if (this.networks) {
        ...
        this.currentUpSpeed = Number.isNaN(upSpeed) ? 0 : upSpeed
        this.currentDownSpeed = Number.isNaN(downSpeed) ? 0 : downSpeed
      }
    },
    covertToKB(bytes) {
    ...
    <div class="network widget-content">
      ...
      <div class="widget-header is-flex mr-0">
        ...
        <div class="is-flex-shrink-0">
          <b-dropdown
            v-if="initNetwork.length > 0" v-model="networkId" :mobile-modal="false" animation="fade1"
          ...
          >
            ...
            <b-dropdown-item
              v-for="(item, index) in initNetwork" :key="`net${index}`" :value="index"
              aria-role="listitem"
            >
              {{ item.name }}
            </b-dropdown-item>
      ...
      <div class="chart-container">
        <VueApexCharts
          ref="chart" :options="chartOptions" :series="networks[networkId]" height="130"
          type="area"
        />
      </div>
      <!-- Chart End -->
