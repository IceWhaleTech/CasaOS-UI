    ...
    getDockerUsage() {
      this.$api.container.getHardwareUsage().then((res) => {
        ...
        this.containerCpuList = res.data.data.map((item) => {
          ...
          return {
            id,
            usage: Number.isNaN(usage) || usage < 0 ? 0 : usage,
            icon: item.icon,
            title: item.title,
          }
        })
        ...
        this.containerRamList = res.data.data.map((item) => {
          ...
          return {
            id,
            usage: Number.isNaN(used_memory) ? 0 : used_memory,
            icon: item.icon,
            title: item.title,
          }
        })
    ...
    <div class="widget-content pb-1">
      ...
      <div class="widget-header is-flex">
        ...
        <div class="widget-icon-button is-flex-shrink-0" @click="showMoreInfo">
          <b-icon :class="{ open: showMore }" class="arrow-btn" icon="right-outline" pack="casa" />
        </div>
      </div>
      <!-- Header End -->
      ...
      <div class="columns is-mobile mt-0 mb-1">
        <div class="column is-half has-text-centered">
          <RadialBar
            :extend-content="power + temperature" :extend-content-clickable="true"
            :percent="parseInt(cpuSeries)" label="CPU" @extend-content-click="changeFormat"
          />
        </div>
        <div class="column is-half has-text-centered">
          <RadialBar
            :extend-content="renderSize(totalMemory)" :percent="parseInt(ramSeries)"
            label="RAM"
          />
        </div>
      </div>
