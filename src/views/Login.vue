    ...
    <div v-if="!isLoading" class="login-panel step4 is-shadow">
      <div class="is-flex is-justify-content-center pb-3 ">
        <div class="has-text-centered">
          <b-image :src-fallback="require('@/assets/img/account/default-avatar.svg')" src="/v1/users/image?path=/var/lib/casaos/1/avatar.png" class="is-128x128" rounded />
        </div>
      </div>
      <b-notification
        v-model="notificationShow" aria-close-label="Close notification" auto-close role="alert"
        type="is-danger"
      >
        {{ message }}
      </b-notification>
      <ValidationObserver v-slot="{ handleSubmit }">
        <ValidationProvider v-slot="{ errors, valid }" name="User" rules="required">
          <b-field
            :label="$t('Username')" :message="errors"
            :type="{ 'is-danger': errors[0], 'is-success': valid }"
            class="mt-3"
          >
            <b-input v-model="username" :autofocus="!username" type="text" @keyup.enter="handleSubmit(login)" />
          </b-field>
        </ValidationProvider>
        <ValidationProvider v-slot="{ errors, valid }" name="Password" rules="required|min:5" vid="password">
          ...
          >
            <b-input
              v-model="password" :autofocus="username" password-reveal
              type="password" @keyup.enter="handleSubmit(login)"
            />
          </b-field>
        </ValidationProvider>
