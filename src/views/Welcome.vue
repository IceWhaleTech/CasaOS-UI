    ...
    <div v-if="!isLoading" v-animate-css="initAni" :class="`step${step}`" class="login-panel is-shadow">
      <div v-if="step === 1" class="has-text-centered">
        <div v-animate-css="s1Ani" class=" is-flex is-justify-content-center">
          <b-image :src="require('@/assets/img/logo/casa-dark.svg')" class="is-128x128 mb-4" />
        </div>
      ...
      <div v-if="step === 2">
        ...
        <div class="is-flex is-justify-content-center ">
          <div class="has-text-centered">
            <b-image
              :src="require('@/assets/img/account/default-avatar.svg')" class="is-128x128"
              rounded
            />
          </div>
        </div>
        <ValidationObserver v-slot="{ handleSubmit }">
          <ValidationProvider v-slot="{ errors, valid }" name="User" rules="required">
            <b-field
              :label="$t('Username')" :message="$t(errors)"
              :type="{ 'is-danger': errors[0], 'is-success': valid }"
            >
              <b-input
                v-model="username" type="text"
                @keyup.enter="handleSubmit(register)"
              />
            </b-field>
          </ValidationProvider>
          <ValidationProvider
            v-slot="{ errors, valid }" name="Password" rules="required|min:5"
            vid="password"
          >
            <b-field
              :label="$t('Password')" :message="$t(errors)"
              :type="{ 'is-danger': errors[0], 'is-success': valid }"
              class="mt-4"
            >
              <b-input
                v-model="password" password-reveal type="password"
                @keyup.enter="handleSubmit(register)"
              />
            </b-field>
          </ValidationProvider>
          <ValidationProvider
            v-slot="{ errors, valid }" name="Password Confirmation"
            rules="required|confirmed:password"
          >
            <b-field
              :label="$t('Confirm Password')" :message="$t(errors)"
              :type="{ 'is-danger': errors[0], 'is-success': valid }" class="mt-4"
            >
              <b-input
                v-model="confirmation" password-reveal type="password"
                @keyup.enter="handleSubmit(register)"
              />
            </b-field>
          </ValidationProvider>
      ...
      <div v-if="step === 3" class="has-text-centered ">
        ...
        <div class="is-flex is-align-items-center is-justify-content-center">
          <LottieAnimation
            :animation-data="require('@/assets/ani/done.json')" :auto-play="true" :loop="false"
            class="animation" @complete="complete"
          />
        </div>
      </div>
    </div>
