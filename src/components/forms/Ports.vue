  ...
  methods: {
    ...
    * */
    invalidPortsInUse(port, type) {
      ...
      // port = port - 0;
      if (type === 'both') {
        return (this.portsInUse?.udp || []).includes(port) || (this.portsInUse?.tcp || []).includes(port)
      }
      if (type) {
        return (this.portsInUse?.[type] || this.portsInUse?.[type.toUpperCase()] || []).includes(`${port}`)
      }
      return false
    },
    ...
    invalidPortsInUseRule(type) {
      if (type === 'both') {
        return (this.portsInUse?.udp || []).concat((this.portsInUse?.tcp || [])).join(',')
      }
      if (type) {
        return (this.portsInUse?.[type] || []).join(',')
      }
    },
  },
}
...
<template>
  <div class="mb-5">
    ...
    <div v-for="(item, index) in items" :key="`port${index}${item.protocol}`" class="port-item mr-4">
      <b-icon class="is-clickable" icon="close-outline" pack="casa" size="is-small" @click.native="removeItem(index)" />
      <ValidationObserver ref="ob" slim>
        <b-field grouped>
          <ValidationProvider
            v-if="showHostPost" v-slot="{ errors, valid }"
            :rules="`yaml_port|not_in_ports:${invalidPortsInUse(item.published, item.protocol)}`" slim
          >
            <!-- Only show title when the first item. -->
            <b-field
              :label="index < 1 ? $t('Host') : ''"
          ...
          <ValidationProvider v-slot="{ errors, valid }" rules="yaml_port" slim>
            <!-- Only show title when the first item. -->
            <b-field
              :label="index < 1 ? $t('Container') : ''"
              :type="{ 'is-danger': errors[0], 'is-success': valid }" expanded
            >
