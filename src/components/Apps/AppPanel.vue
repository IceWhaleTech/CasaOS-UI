...
import { debounce, isNull, orderBy, uniq } from 'lodash-es'
  ...
  methods: {
    ...
    async getStoreRecommend() {
      try {
        ...
        console.error('load recommend error', error)
      }
    },
    ...
    async getStoreList() {
      ...
      try {
        ...
        console.error('load store list error', e)
      }
      this.isLoading = false
    },
    ...
    showTerminalPanel() {
      this.$openAPI.appManagement.compose
        ...
        .catch((err) => {
          console.error('$openAPI.appManagement.compose.composeAppContainers', err.response)
        })
    },
    ...
    async askInstallationLocation() {
      try {
        ...
        console.error(`${err} in askInstallationLocation`)
      }
    },
    ...
    async submitInstallationLocation(val) {
      ...
      this.$api.container
        ...
        .catch((err) => {
          this.isLoading = false
          console.error(`${err} in submitInstallationLocation`)
          this.$buefy.toast.open({
            message: err.message,
            type: 'is-danger',
          })
    ...
    <template v-else>
      ...
      <AppSideBar v-model="sidebarOpen" :overlay="true" :right="true" position="absolute">
        <template #default="{ close }">
          <AppDetailInfo
            ...
            @switch-app-config-content="switchAppConfigContent"
          />
        </template>
      </AppSideBar>
      <!-- Sidebar End -->
        ...
        <template v-if="!isLoadError">
          <template v-if="recommendList.length > 0">
            ...
            <div class="is-relative featured-app b-line">
              <Swiper :options="featureSwiperOptions" class="swiper">
                <SwiperSlide
          ...
          <div class="is-flex mt-5 mb-5 is-justify-content-center">
            ...
            <AppStoreSourceManagement
              ...
              class="ml-2"
              @refresh-app-store="getStoreList"
              @refresh-size="refreshAppStoreSourceManagementSizeStatus"
            />
          </div>
      ...
      <template v-if="currentSlide === 1">
        <ComposeConfig
          ...
          @update-docker-compose-commands="updateDockerComposeCommands"
          @update-docker-compose-service-name="updateDockerComposeServiceName"
          @update-main-name="name => (currentInstallId = name)"
        />
