...
<template>
  <div v-on-click-outside="onClickOutsideHandler">
    <Transition mode="out-in" name="management-change" @after-enter="activeInput">
      <b-button
        v-if="componentState === 'first_add_state'" icon-pack="casa" icon-left="plus-outline"
      ...
      <div v-else-if="componentState === 'second_list_state'" key="2">
        <b-dropdown
          ref="sourceDorpRef" aria-role="menu" position="is-bottom-left"
          class="file-dropdown source-dropdown" animation="fade1" :close-on-click="false"
        >
          ...
          <b-dropdown-item v-for="item in sourceList" :key="item.id" aria-role="menu-item" custom>
            <p :ref="`removeButton${item.id}`" class="is-flex is-align-items-center">
              ...
              <template v-if="operationSourceName !== item.id">
                <b-icon
                  class="close-button" custom-size="casa-16px" icon="trash-outline" pack="casa"
                  @click.native="operationSourceName = item.id"
                />
              </template>
              <template v-else>
                <b-icon
                  class="close-button" custom-size="casa-16px" icon="close-outline" pack="casa"
                  @click.native="operationSourceName = -1"
                />
                <b-icon
                  class="close-button" custom-size="casa-16px" icon="check-outline" pack="casa"
      ...
      <div v-else-if="componentState === 'active_input_state'" key="3" class="is-flex is-align-items-center">
        <b-field class="mb-0">
          <b-input
            ref="inputSourceURL" v-model="url" class="_sources_input" :disabled="addLoadingState"
            @keyup.enter.native="registerAppStore(url)"
          />
          <b-tooltip
            label="Get more apps" position="is-bottom" class="add-tooltip" type="is-dark"
            :class="{ disabled: addLoadingState }"
          >
            <b-icon
              class="is-clickable" icon="question-outline" pack="casa" size="is-small"
              @click.native="redirectURL"
            />
          </b-tooltip>
          <p class="control">
            <b-button class="_sources_input" :loading="addLoadingState" @click="registerAppStore(url)">
              {{
                $t("Add") }}
            </b-button>
          </p>
