...
import { concat, isEqual, last, xor } from 'lodash-es'
...
export default {
  ...
  methods: {
     ...
     * @param {object} item - The app item object.
     * @param {boolean} isCasa - Indicates if the app is a Casa app.
     * @return {*}
     */
    ...
    >
      ...
      <template v-if="!isLoading">
        <div v-for="item in appList" :id="`app-${item.name}`" :key="`app-${item.name}`" class="handle">
          <AppCard
            :item="item"
            @config-app="showConfigPanel"
            @import-app="showContainerPanel"
