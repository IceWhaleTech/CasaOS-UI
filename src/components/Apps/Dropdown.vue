...
export default {
  ...
  methods: {
    ...
    clickedOutside(event) {
      if (!this.cancelOptions.includes('outside'))
        return
      if (this.inline)
        return
    ...
    keyPress({ key }) {
      if (this.isActive && (key === 'Escape' || key === 'Esc')) {
        if (!this.cancelOptions.includes('escape'))
          return
        this.isActive = false
      }
    },
    ...
    onContextMenu() {
      if (!this.triggers.includes('contextmenu'))
        return
      this.toggle()
    },
    onHover() {
...
<template>
  <div ref="dropdown" :class="rootClasses" class="dropdown dropdown-menu-animation" @mouseleave="isHoverable = false">
    <div
      v-if="!inline" ref="trigger" :tabindex="disabled ? false : triggerTabindex" aria-haspopup="true"
    ...
    <transition :name="animation">
      <div
        v-show="(!disabled && (isActive || isHoverable)) || inline" ref="dropdownMenu" v-trap-focus="trapFocus"
