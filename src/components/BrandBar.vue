...
export default {
  ...
  methods: {
    async parseFeed() {
      ...
      const feed = await parser.parseURL(`https://blog.casaos.io/feed/tag/dashboard/?key=${stringify}`)
    ...
    <span v-else class="window ml-4">
      <ul :style="{ '--time': `${5 * line}s`, '--perc': perc, '--line': line }" class="scroll">
        <li v-for="(item, key) in rss" :key="key" class="has-text-left" @click="$messageBus('connect_news')">
          <a
            class="intro-text is-clickable" target="_blank" rel="noopener noreferrer"
