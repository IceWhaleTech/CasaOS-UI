      ...
      <div class="popper">
        <div class="is-flex is-align-items-center">
          ...
          <div class=" is-flex-shrink-0 mr-2">
            <b-button
              :label="$t('cancel-all')" :loading="isLoading" class="mr-2" rounded size="is-small"
              type="is-primary is-light" @click="cancel"
            />
          </div>
        </div>

        <OperationStatusItem v-for="task in taskList" :key="task.id" :data="task" />
      </div>
      <b-button slot="reference" rounded size="is-small" type="is-primary is-light">
        <b-icon custom-class="mdi-spin" custom-size=" mdi-18px" icon="autorenew" />
      </b-button>
    </Popper>
  </div>
</template>
