...
<template>
  <div class="action-btn">
    <b-dropdown
      id="location-drop" animation="fade1" append-to-body aria-role="list"
      class="file-dropdown"
      close-on-click position="is-bottom-left"
    >
      <template #trigger>
        <p class=" is-clickable" role="button">
          <b-icon custom-size="mdi-18px" icon="plus" />
        </p>
      </template>
      ...
      <b-dropdown-item
        v-for="(driver, index) in platforms" :key="`${index}platform`" aria-role="menuitem"
        @click="auth(driver)"
      >
        <div class="is-flex is-align-items-center">
          <div class="img-container mr-1">
            <b-image :src="driver.icon" class="is-16x16" />
          </div>
