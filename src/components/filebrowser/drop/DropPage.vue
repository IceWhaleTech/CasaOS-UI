...
<template>
  <div id="drop-page" class="content is-flex is-flex-direction-column is-flex-grow-1">
    ...
    <div :class="areaClass" :style="cssVariables" class="action-area is-flex-grow-1 is-relative">
      <div class="contents">
        <!-- Cricle BG Start -->
        <DropBg v-if="isDesktop" />
        <!-- Circle Bg End -->
        <transition-group class="contents" name="list-complete" tag="div">
          <DropItem
            v-for="(item, index) in peersArray" :key="item.id" :center="centerPos"
        ...
        <DropAddButton
          v-if="showAddButton && peersArray.length === 1 && isDesktop" :center="centerPos"
      ...
      <DropCenterIcon v-if="!isMobile" />
      <!-- Bottom Center Icons End -->

      <DropAddButton
        v-if="isDesktop ? showAddButton && peersArray.length > 1 : true" :center="centerPos"
