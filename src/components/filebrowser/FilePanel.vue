    ...
    <div class="modal-card">
      ...
      <section class="modal-card-body is-flex">
        ...
        <transition name="c-zoom-in">
          <component
            :is="panelType" v-if="isShowDetial" ref="previewPanel" :item="currentItem" :list="listData"
            @close="isShowDetial = false; isModalOpen = false;"
          />
        </transition>
        <!-- Viewer End -->
        ...
        <template>
          <!-- NavBar Start -->
          <b-sidebar
            ref="sidebar" :open.sync="isSideBarOpen" :overlay="isMobile" :position="sideBarPosition"
            fullheight
          >
            <div class="nav-bar is-flex is-flex-direction-column">
              <div class="is-flex-grow-1 is-flex-shrink-1 nav-bar-top scrollbars-light">
                ...
                <div class="files-section">
                  ...
                  <div class="is-flex is-align-items-center pt-3">
                    ...
                    <div
                      v-show="hasMergerFunction" class="is-flex-shrink-0 mr-5 is-clickable"
                  ...
                  <div class="list-container scrollbars-light pt-0 is-flex-grow-1">
                    <TreeList
                      :auto-load="true" :is-active="pageType === `file`"
                      :path="rootPath"
                    />
                  </div>
                </div>
              ...
              <div class="bottom-area">
                <DropEntryButton
                  :active="pageType === `drop`" :title="$t('FilesDrop')"
                  @open="showDropPage"
                />
                <ShareEntryButton
          ...
          <template v-if="pageType === `file`">
            <div class="content is-flex-grow-1">
              <Uploader ref="uploader" :options="options" class="uploader-example">
                ...
                <header :class="{ 'is-flex-wrap-wrap': isMobile }" class="modal-card-head">
                  <div id="bread-container" class="is-flex-grow-1 is-flex breadcrumb-container">
                    ...
                    <SidebarMenuButton />
                    <!-- SideBar Button End -->
                    <FileBreadcrumb v-if="!isMobile" />
                    <!-- <b-input placeholder="Search in folder..." size="is-small" rounded></b-input> -->
                  </div>
                  <div class="is-flex is-align-items-center">
                    <!-- Paste Button Start -->
                    <b-button
                      v-if="hasPasteData" :label="$t('Paste')" :loading="isPasting" class="mr-3"
                    ...
                    >
                      <div class="close-button" @click="$emit('close')">
                        <b-icon icon="close-outline" pack="casa" />
                      </div>
                    </div>
                    <!--  Close Button End -->
                  </div>
                ...
                <div v-if="listData.length > 0" class="tool-bar is-flex mb-2 mt-2 is-flex-shrink-0">
                  <div class="is-flex-grow-1 has-text-left is-flex is-align-items-center">
                    <b-field class="ml-1 is-flex is-size-14px mb-0" expanded>
                      <b-checkbox
                        v-model="isSelectAll" :class="selectState" size="is-small"
                        @input="handleSelect"
                      >
                        {{
                          selectState !== "none"
                ...
                <div id="dropTarget" class="is-flex-grow-1 is-flex over is-justify-content-center ">
                  ...
                  >
                    <div class="mb-6">
                      <div class="upload-icon">
                        <b-icon icon="arrow-up" size="is-medium" type="is-white" />
                      </div>
                  ...
                  <component
                    :is="listView" ref="listview" v-model="listData" :is-loading="isLoading"
                    @change="handelListChange" @gotoFolder="getFileList" @reload="reload"
                    @showDetailModal="showDetailModal"
                  >
                    <EmptyHolder
                      v-if="isEmpty" @newFile="showNewFileModal"
                      @newFolder="showNewFolderModal"
                    />
                    <ErrorHolder v-else :error="errorMsg" />
                  </component>
                </div>
                <!-- List View End -->
                ...
                <div v-show="showUploadList" class="upload-list">
                  <b-collapse
                    v-model="openUploadList" animation="slide1"
                    aria-id="contentIdForA11y3" class="card"
                  >
                    <template #trigger>
                      <div
                        :aria-expanded="openUploadList" aria-controls="contentIdForA11y3"
                        class="card-header" role="button"
                      >
                        <p class="card-header-title">
                          <b-icon
                            :icon="openUploadList ? 'down-outline' : 'up-outline'
                            " pack="casa" class="mr-2"
                          />
              ...
              <!-- Toolbar Start -->
              <OperationToolbar
                v-model="isToolbarShow" @close="handleClose" @copy="handleCopy"
          ...
          <template v-else-if="pageType === `share`">
            <ShareListPage ref="shareList" @close="$emit('close')" />
          </template>
          <!-- Share Page End -->

          <!-- Drop Page Start -->
          <template v-else-if="pageType === `drop`">
            <DropPage @close="$emit('close')" />
          </template>
          <!-- Drop Page End -->
        </template>
        <!-- Main Content End -->
