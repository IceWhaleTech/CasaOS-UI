...
<template>
  <div>
    <div class="ul">
      ...
      <div v-if="hasMergerFunction" class="li">
        <div
          :class="{ active: isActived }" class="is-flex list-item new-list-item"
        ...
        >
          ...
          <div v-if="testMergeMiss > 0" class="is-flex is-align-items-center">
            <b-icon class="warn" custom-size="casa-16px" icon="danger-outline" pack="casa" />
          </div>
        </div>
        <ul v-show="dorpdown && mergeStorageList.length > 0">
          <TreeListItem
            v-for="item in mergeStorageList" :key="item.path" :is-active="isActive"
            :item="item"
          />
        </ul>
      </div>
      ...
      <TreeListItem
        v-for="item in localStorageList" :key="item.path" :is-active="isActive"
        :item="item"
      />
      ...
      <TreeListItem
        v-for="item in networkStorageList" :key="item.path" :is-active="isActive" :item="item"
        icon-name="eject" @rightIconClick="umountNetwork"
      />
      <!-- Network Storage List End -->

      <!-- USB List Start -->
      <TreeListItem
        v-for="item in usbStorageList" :key="item.path" :is-active="isActive" :item="item"
        icon-name="eject" @rightIconClick="umountUsb"
      />
      <!-- USB List End -->

      <!-- Cloud List Start -->
      <TreeListItem
        v-for="item in cloudStorageList" :key="item.path" :icon-type="item.icon_type"
        :is-active="isActive" :item="item" icon-name="eject" @rightIconClick="umountCloud"
      />
      <!-- Cloud List End -->
    </div>
