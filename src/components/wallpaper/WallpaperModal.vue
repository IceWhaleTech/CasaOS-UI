<script>
import wallpaper01 from '@/assets/background/wallpaper01.jpg'

import wallpaper02 from '@/assets/background/wallpaper02.jpg'
    ...
    <section class="modal-card-body ">
      <div class="preview-card">
        <div :style="backgroundStyleObj" class="preview-image">
          <b-image :src="require('@/assets/background/preview-widget.svg')" class="preview-widget" />
          <b-image :src="require('@/assets/background/blank.png')" />
        </div>
      </div>
      ...
      <div class="columns mt-5 mb-5 is-variable is-2">
        <div v-for="(item, index) in wallpaperItems" :key="`wallpaper${index}`" class="column">
          <div
            :class="{ active: checkActive(item.path) }" class="image-list-item is-clickable"
            @click="changeWallpaper(item.path)"
          >
            <b-image :src="item.path" />
          </div>
        </div>
        ...
        <div class="column is-relative is-one-quarter">
          <div :class="{ active: checkActiveFrom('Upload') }" class="upload-button-container is-clickable">
            ...
            >
              <b-icon icon="picture-upload-outline" pack="casa" size="is-large" />
            </div>
            <b-loading v-model="isUpLoading" :can-cancel="false" :is-full-page="false" />
          </div>
        </div>
      </div>
    </section>
...
.preview-card {
  ...
  background-image: linear-gradient(to right, #fff, #fff),
    linear-gradient(108.27deg, rgba(57, 60, 64, 1) 1.44%, rgba(92, 96, 102, 1) 55.8%, rgba(34, 36, 38, 1) 110.95%);
