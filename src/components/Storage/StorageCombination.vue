...
<template>
  <div v-show="showCombination" class="mb-5 mt-2 pt-5 pb-5 border-1 combination-box">
    <div class="is-relative is-flex is-justify-content-center top--2rem">
      <div class="pr-4 pl-4 has-background-white">
        CasaOS HD
      </div>
    </div>
    <div class="is-flex ">
      <div class="is-flex-grow-1">
        <div v-for="(item, index) in storageData" :key="`mergeStorage${index}`" class="ml-5 is-flex mb-3">
          <div class="header-icon">
            <b-icon v-show="!item.name" class="warn is-16x16" icon="danger" pack="casa" />
          ...
          <div class="ml-3 is-flex-grow-1 is-flex is-align-items-center">
            <div>
              <h4 class="title is-size-14px mb-0 has-text-left one-line">
                ...
                <b-tag v-if="item.isSystem" class="ml-2">
                  CasaOS
                </b-tag>
              </h4>
      ...
      <div class="mr-5 is-flex is-flex-direction-column-reverse is-justify-content-space-between">
        ...
        <div class="is-flex is-flex-direction-row-reverse">
          <b-button
            :type="type" class="width" rounded size="is-small"
            @click="showStorageSettingsModal"
          >
            {{ $t('Merge Storages') }}
          </b-button>
          <CToolTip is-block modal="is-success" />
        </div>
      </div>
    </div>
    <b-progress
      :type="usePercent | getProgressType" :value="usePercent" class="ml-5 mr-5"
      size="is-small"
    />
  </div>
</template>
