  ...
  <div class="mb-4 pb-3 background-item is-flex is-flex-direction-column">
    <div class="is-flex mt-3 mr-3 mb-3 ml-3">
      <div class="header-icon">
        <b-image :src="require('@/assets/img/storage/storage.png')" class="is-64x64" />
      </div>
      <div class="ml-3 is-flex-grow-1 is-flex is-align-items-center">
        <div>
          <h4 class="mb-0 has-text-left one-line has-text-emphasis-02 is-flex is-align-items-center">
            {{ item.name }}
            <b-tag v-if="item.isSystem" class="ml-2 has-text-full-04">
              OS
            </b-tag>
          </h4>
          ...
          <p class="has-text-left has-text-full-04 has-text-grey-light mt-1">
            ...
            <b-tooltip
              ...
              append-to-body
            >
              <b-icon class="mr-2 " icon="question-outline" pack="casa" size="is-small" />
            </b-tooltip>
          </p>
      ...
      <div v-if="!item.isSystem" class="is-flex is-align-items-center b-group">
        <b-button
          :disabled="isRemoving" :loading="isFormating" :type="isFormating ? 'is-primary' : ''"
          rounded size="is-small" @click="formatStorage(item.path, item.mount_point)"
        >
          {{ $t('Format') }}
        </b-button>
        <b-button
          :disabled="isFormating" :loading="isRemoving" :type="isRemoving ? 'is-primary' : ''" class="ml-2"
          rounded size="is-small" @click="removeStorage(item.disk)"
        >
          {{ $t('Remove') }}
        </b-button>
      </div>
    ...
    <b-progress
      :type="item.usePercent | getProgressType" :value="item.usePercent || 0"
      class="ml-3 mr-3" size="is-small"
    />
  </div>
</template>
