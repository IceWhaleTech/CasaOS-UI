...
<template>
  <div :class="{ '_max-width-320': currentStep === 1 || currentStep === 3 || currentStep === 4 }" class="modal-card">
    ...
    <header :class="{ 'pri-head': currentStep <= 2 }" class="modal-card-head ">
      <b-icon
        v-if="currentStep === 1" class="mr-2 has-text-red" custom-class="_has-text-red-default"
        icon="danger"
        pack="casa" size="is-20"
      />
      <div class="is-flex-grow-1">
        <h3 class=" title is-3">
          {{ $t(title) }}
          <CToolTip v-if="currentStep === 0" modal="is-success" />
        </h3>
      </div>
    ...
    >
      ...
      <div v-for="(item, index) in storageData" :key="item.path + index" class="is-flex mb-1 radius _height-40">
        <div class="ml-2 mr-1 is-flex is-align-items-center _has-text-gray-600">
          <b-icon icon="storage-other" pack="casa" size="is-20" />
        </div>
        ...
        <b-checkbox
          v-model="checkBoxGroup" :disabled="item.persistedIn !== 'casaos' || item.isSystem"
          :native-value="item.uuid" class="mr-2"
        />
      </div>

      <div
        v-for="(item, index) in storageMissData" :key="item.path + index"
      ...
      >
        ...
        <div class="is-flex is-flex-shrink-0 is-flex-direction-column is-justify-content-center mr-2">
          <span class="is-flex is-align-items-center has-text-danger small-font">
            <b-icon class="warn" custom-size="casa-16px" icon="danger" pack="casa" />
            {{ $t('Missing') }}
          </span>
        </div>
        <b-checkbox v-model="checkBoxMissGroup" :native-value="item.path" class="mr-2" />
      </div>
    </section>
    ...
    >
      <div class="is-flex left ml-3 mr-2 is-align-items-center">
        <b-icon class="is-16x16" custom-size="casa-19px" icon="danger" pack="casa" />
      </div>
    ...
    >
      <div class="is-flex left ml-3 mr-2 is-align-items-center">
        <b-icon custom-size="casa-19px" icon="danger" pack="casa" />
      </div>
    ...
    >
      ...
      <template v-if="currentStep === 2">
        ...
        <b-input
          ref="inputPassword" v-model="password" class="mt-4" password-reveal
          @keyup.enter="verifyOperate(password)"
        />
      </template>
      <div v-if="currentStep === 3" class="is-flex is-align-items-center font">
        <div class="message-danger left mr-2 is-flex is-align-items-center">
          <b-icon class="is-38x38" custom-size="is-size-2" icon="danger" pack="casa" />
        </div>
    ...
    <footer :class="{ 't-line': currentStep <= 2 }" class="modal-card-foot is-flex is-align-items-center ">
      ...
      <div class="mr-4">
        <b-button
          v-show="currentStep > 2 || currentStep === 1" :label="$t('Cancel')"
          class="_has-background-gray-100 _radius-line" expaned @click="currentStep = 0"
        />
      </div>
      <div>
        <b-button
          v-show="currentStep === 0" :label="$t(affirm)" :loading="isConnecting" expaned rounded
          type="is-primary" @click="test"
        />
        <b-button
          v-show="currentStep === 1" :label="$t(affirm)" :loading="isConnecting"
          class="_has-background-red-default _radius-line _has-text-white" expaned
          @click="currentStep = 2"
        />
        <b-button
          v-show="currentStep === 2" :label="$t(affirm)" :loading="isConnecting" expaned rounded
          type="is-primary" @click="verifyOperate(password)"
        />
        <b-button
          v-show="currentStep === 3" :label="$t(affirm)" :loading="isConnecting" expaned rounded
          type="is-primary" @click="restart"
        />
        <b-button
          v-show="currentStep === 4" :label="$t(affirm)" :loading="isConnecting" expaned rounded
          type="is-primary" @click="restart"
        />
      </div>
    </footer>
