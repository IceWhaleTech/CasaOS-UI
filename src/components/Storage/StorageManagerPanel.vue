...
export default {
  ...
  methods: {
     ...
     * @param {int} index index of select
     * @return {void}
     */
    onDiskChoose(index) {
          ...
          <div v-if="activeTab === 0" class="create-container">
            ...
            >
              ...
              <template #reference>
                <div>
                  <b-button
                    :disabled="unDiskData.length === 0" :type="state_createstorage_operability"
                    class="o" rounded size="is-small" @click="showCreate"
                  >
                    {{ $t('Create Storage') }}
                  </b-button>
                </div>
        ...
        <div v-if="creatIsShow" class="is-flex-grow-1 is-relative">
          <ValidationObserver ref="ob1">
            <ValidationProvider v-slot="{ errors, valid }" name="StorageName" rules="required">
              <b-field
                :label="$t('Storage Name')" :message="$t(errors)"
                :type="{ 'is-danger': errors[0], 'is-success': valid }"
              >
                <b-input
                  ref="inputs" v-model="createStorageName"
              ...
              <div class="media">
                <div class="media-left">
                  <span class="icon is-large is-danger"><i
                    class="mdi mdi-alert-circle mdi-48px"
                  /></span>
                </div>
    ...
    <section v-if="isCreating" class="modal-card-body ">
      <div class="installing-warpper mt-6 mb-6">
        <div class="is-flex is-align-items-center is-justify-content-center mb-5">
          <LottieAnimation
            :animation-data="require('@/assets/ani/creating.json')" :auto-play="true" :loop="true"
    ...
    <footer v-if="!isCreating && activeTab === 0" class="modal-card-foot is-flex-shrink-0 is-flex is-align-items-center">
      <template v-if="creatIsShow">
        ...
        <div>
          ...
          <b-button
            :label="$t('Format and Create')" :loading="isValiding"
            :type="createStorageType === 'format' ? 'is-primary' : ''" rounded @click="createStorge(true)"
          />
          <b-button
            v-if="createStorageType === 'mountable'" :label="$t('Create')" :loading="isValiding" rounded
      ...
      <template v-else-if="!mergeConbinationsStorageData.length">
        ...
        <div class="is-flex is-flex-direction-row-reverse">
          <b-button
            :type="state_mainstorage_operability" class="width" rounded
